...
% Short title
\shorttitle{Onboard Hyperspectral Segmentation in Downlink-Constrained LEO SmallSat Constellations}

% Short author
\shortauthors{Keon-Woo, et al}

% Main title of the paper
%\title [mode = title]{Mission Architecture for Result-Centric Hyperspectral Downlink in Low-Earth-Orbit Constellations}     
\title [mode = title]{Mamba-Transformer Fusion for Onboard Hyperspectral Segmentation in Downlink-Constrained LEO SmallSat Constellations Operations}      
% Title footnote mark
% eg: \tnotemark[1]
% \tnotemark[1,2]
...
\author[4]{Keon-Woo Koo\orcidlink{0000-0001-9468-368x}}
%\fnmark[1]
%\corref{cor1}
\ead{rnrjsdnspace@skku.edu }
\author[1]{Haruna H. Mohammed\orcidlink{0009-0004-5825-9784}}
%\fnmark[1]
%\corref{cor1}
...
\author[2]{Myungchul Choi\orcidlink{0009-0004-3801-6232}}
%\fnmark[1]
%\corref{cor1}
\ead{ myho0103@ust.ac.kr}
\author[3]{Jaehwan Park\orcidlink{0009-0008-9423-6458}}
%\fnmark[1]
%\corref{cor1}
\ead{abcjack41@gmail.com}
\author[1]{Hyochoong Bang\orcidlink{0000-0001-6016-8102}}
%\fnmark[3]
\corref{cor1}
\ead{hcbang@kaist.ac.kr}
\author[4]{Jaekwang Kim\orcidlink{0000-0001-5174-0074}}
%\fnmark[1]
%\corref{cor1}
\ead{linux@skku.edu}
\author[2]{Dongho Yun\orcidlink{0000-0001-9757-2571}}
%\fnmark[1]
%\corref{cor1}
\ead{lipo123@ust.ac.kr}
...
\affiliation[2]{organization={Robotics, University of Science and Technology},%Department and Organization
            addressline={217}, 
            city={Gajeong-ro, Yuseong-gu, Daejeon},
            postcode={34113}, 
            country={Republic of Korea}}
...
\begin{abstract}
	Earth-observation constellations are increasingly constrained by intermittent downlink opportunities rather than sensing capability, as hyperspectral payloads generate multi-gigabyte cubes that exceed available contact-window bandwidth. Existing workflows transmit raw imagery before analysis, increasing alert latency and incurring a high contact demand. This paper proposes Mamba-OpticSatNet, a result-centric mission architecture that performs onboard semantic segmentation, detects regions of interest (ROI), transmits a compact semantic alert, stores raw data locally, and defers evidence retrieval until a subsequent contact.
	The Mamba-OpticSatNet architecture is evaluated in two configurations: Full is accuracy-oriented, fusing parallel Transformer and selective state-space branches through an adaptive gate, while Lite is deployment-oriented, employing a lightweight Mamba-residual stream similar to MRNet's hybrid CNN-Mamba design for edge-constrained detection that reduces token-processing cost and complexity. On a four-class derivative of OHID-1, Full achieved a mean Intersection-over-Union (mIoU) of 0.908, whereas Lite achieved 0.877 with 0.288 M parameters and 0.066 GMACs. On an NVIDIA Jetson AGX Orin at 25 W, Lite processed a hyperspectral cube in 1.698 seconds at 42.44 J per inference.
	In an STK scenario with 20 satellites at 450 km and 76° inclination, average ground-station access was 302 s. At an illustrative 1 Mbit/s S-band application-layer throughput, a 1.64 GB cube required 13,120 s and 44 mean-duration contacts, whereas a 100 kB semantic alert required 0.8 s and one contact, reducing initial alert-downlink volume by 99.9939 \%. These results support an alert-first, evidence-later concept of operations, while absolute mission latency remains dependent on contact chronology, scheduling, protocol overhead, and link-budget conditions.
\end{abstract}
...
\begin{figure*}[pos=hbp!]
	\centering\includegraphics[width=0.9\textwidth]{figs/fig10}
...
\begin{eqnarray}\label{eq:lantecy}
	\begin{aligned}
		L_{post}(m,p) \approx T_{proc}(m,p) + T_{wait}(m,p) + \frac{B_p}{R_{\mathrm{eff}}}\\
		 + T_{ground}(p)
	\end{aligned}
\end{eqnarray}
...
\begin{figure*}[pos=hbp!]
	\centering\includegraphics[width=0.9\textwidth]{figs/fig20}
...
\begin{figure*}[pos=hbp!]
	\centering\includegraphics[width=0.9\textwidth]{figs/fig30}
...
For the benchmark input configuration, full Mamba-OpticSatNet requires 0.240 giga multiply–accumulate operations (GMACs) and contains 0.681 million trainable parameters. Mamba-OpticSatNet(Lite) requires 0.066 GMACs and contains 0.288 million trainable parameters. The relative reductions from Full to Lite are
\begin{eqnarray}
	\begin{aligned}
...
\subsection{Embedded hardware configuration}
Training and patch-level benchmarking were performed on the same workstation. All compared models were evaluated under the same hardware and software environment. The reported workstation inference time represents latency per 32×32 patch and is used only for relative comparison within the segmentation benchmark.

All training and patch-level benchmarking were performed on a single workstation running Linux 6.8.0-90-generic (x86\_64, glibc 2.35), equipped with an eight-core, sixteen-thread x86\_64 CPU, 30.51~GB of system memory, and an NVIDIA GeForce RTX 5090 GPU with 31.36~GB of device memory. The software environment comprised Python 3.10.19 (Conda-forge build, GCC 14.3.0), NVIDIA driver 570.195.03, CUDA 12.8 with cuDNN 9.17.1, and PyTorch 2.11.0.dev20260207+cu128. All compared models were trained and evaluated under this identical hardware and software configuration.
%\begin{table}[htb]
%	\centering
...
%\end{table}
Embedded evaluation was conducted using Mamba-OpticSatNet(Lite) on NVIDIA Jetson Xavier NX, Jetson AGX Xavier, Jetson Orin NX, and Jetson AGX Orin platforms. Cube-level latency, reported average power, estimated energy per inference, and throughput are presented in Figure \ref{fig:latencyconfusion} (a).
...
The constellation and ground-contact screening scenario was implemented using Ansys Systems Tool Kit (STK) as shown in Figure \ref{fig:stk}. Table \ref{tab:stk_config} summarizes both the orbital configuration and aggregate access-duration statistics.
\begin{table}[htb]
	\centering
	\caption{Orbital smallsat constellation configuration and aggregate ground-contact results.}
...
Table \ref{table:operating_trade} summarizes the segmentation accuracy and patch-level computational characteristics of Mamba-OpticSatNet and the baseline models on the four-class OHID-1 derivative benchmark.
...
Figure \ref{fig:accuracycomp} visualizes the relationship between segmentation accuracy and architecture-level resource requirements. The left panel compares mIoU with GMACs, while the right panel compares parameter count with mIoU.
...
\begin{figure*}[pos=hbp!]
	...
	\label{fig:segmentcomp}
\end{figure*}
\subsubsection{Mamba-OpticSatNet (Full) vs. (Lite) config}
...
\begin{table}[htb]
	...
	\end{tabularx}
\end{table}
Table \ref{table:operating_trade} summarizes the resulting accuracy–efficiency trade-off between the Full and Lite configurations. Relative to Full, Lite incurs an absolute decrease of 0.031 in mIoU and 0.118 in C2 Urban IoU, while reducing GMACs by 72.5 \% and parameter count by 57.7 \%. Lite also reduces workstation patch latency from 6.016 ms to 0.384 ms, corresponding to an approximately 15.7× speedup. These results indicate that the Lite configuration achieves substantial reductions in computational and model complexity at the cost of a relatively small decrease in overall segmentation accuracy, although the larger degradation in C2 Urban IoU indicates a more pronounced trade-off for the most challenging class. Whether this trade-off is acceptable depends on the operational consequence of missed or under-segmented urban regions in the target mission.
...
Figure \ref{fig:mambalite} summarizes the cube-level embedded inference performance of Mamba-OpticSatNet (Lite) across the evaluated Jetson platforms in terms of latency, average power, estimated energy per inference, and throughput. Energy per inference is evaluated from mean power and measured processing time as
\begin{eqnarray}
...
\begin{figure*}[pos=hbp!]
	...
	\begin{tabular}{cccc}
	\includegraphics[width=0.4\textwidth]{figs/fig_energy} &
	\includegraphics[width=0.45\textwidth,height=150px]{figs/fig_confusion.pdf} &\\
	\textbf{(a)} Energy versus Jetson platforms & \textbf{(b)}  Normalized confusion matrix &\\[6pt]
	\end{tabular}
	\end{minipage}
	\caption{Cube-level performance of Mamaba-OpticSatNet energy against latency across various Jetson platforms and normalized confusion matrix showing true class against predicted one}
	\label{fig:latencyconfusion}
\end{figure*}
%\begin{table*}[htb]
%	\centering
...
\subsubsection{Data communication analysis}
...
\begin{figure*}[pos=hbp!]
	\centering
	\includegraphics[width=0.9\textwidth,height=170px]{figs/Constellation.png}
	\caption{Trajectories of the defined spacecraft constellation orbits}
	\label{fig:stk}
\end{figure*}
